...
| `mail_protocol_imap`| false | If t`true` enable IMAP on port 143/tcp with STARTTLS supported. Not recommended.
| `mail_protocol_imaps`| true | If t`true` enable IMAPS on port 993/tcp with TLS enabled.
| `mail_protocol_pop3`| false | If t`true` enable POP3 on port 110/tcp with STARTTLS supported. Not recommended.
| `mail_protocol_pop3s`| false | If t`true` enable POP3S on port 995/tcp with TLS enabled.
| `mail_protocol_smtp`| false | If t`true` enable SMTP on port 25/tcp with STARTTLS supported. Not recommended.
| `mail_protocol_smtps`| true | If t`true` enable SMTPS on port 465/tcp with TLS enabled.
| `mail_protocol_submission`| true | If t`true` enable SMTP-Submission on port 587/tcp with STARTTLS enabled.
