...
| `common_dnf_proxy`| | URL of a proxy server to connect through The expected format of this option is `<scheme>://<ip-or-hostname>[:port]`. If you configured another host with the [**squid**](squid.md) role, you may set this value to `http://<squid-host-ip-or-hostname>:<squid-port>`.
